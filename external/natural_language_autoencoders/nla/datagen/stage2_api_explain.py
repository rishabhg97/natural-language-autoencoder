...
def _extract_and_clean(raw: str, pattern: str) -> str | None:
    ...
    if not matches:
        return None
    # Reasoning models often quote or deliberate about the requested XML-ish
    # format before giving the final answer. Use the last complete block so a
    # reasoning trace does not train on an earlier quoted template.
