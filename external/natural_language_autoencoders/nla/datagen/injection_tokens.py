...
The actor prompt contains a marker token (e.g. ㊗ or 々) whose embedding is replaced
...
We first try CJK enclosed-ideographs (U+3200–U+33FF), matching the original
NLA implementation. Some tokenizers do not have single-token entries in that
block, so we then try rare CJK symbols before broader CJK ranges. Results are
cached to a committed YAML so repeat runs with the same tokenizer get the
same ID.
...
# CJK Enclosed Letters and Months / CJK Compatibility blocks are tried first.
# ㊗ (U+3297 "circled ideograph congratulation") lives there. Nano's tokenizer
# currently has no single-token entry in that block, so keep ordered fallbacks
# that are still single-codepoint and essentially absent from English text.
_INJECTION_RANGES = (
    ("cjk_enclosed", 0x3200, 0x33FF),
    # Skip U+3000 ideographic space plus common CJK comma/period. U+3005 "々"
    # is a good Nano fallback: a CJK iteration mark, single token, not English.
    ("cjk_symbols", 0x3005, 0x303F),
...
def find_injection_token(tokenizer: Any) -> tuple[str, int]:
    ...
    tried = []
    for block_name, lo, hi in _INJECTION_RANGES:
        tried.append(f"{block_name}=U+{lo:04X}–U+{hi:04X}")
        for codepoint in range(lo, hi + 1):
            char = chr(codepoint)
            ids = _tokenize_one(tokenizer, char)
            if len(ids) == 1 and (unk_token_id is None or ids[0] != unk_token_id):
                cache[key] = {"char": char, "token_id": ids[0], "block": block_name}
                _save_cache(cache)
                return char, ids[0]

    raise AssertionError(
        f"no single-token CJK char found in {', '.join(tried)} for tokenizer "
