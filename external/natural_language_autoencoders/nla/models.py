...
# Nano/Nemotron-H: backbone.embeddings.weight
_EMBED_KEY_SUFFIXES = (
    "embed_tokens.weight",
    "wte.weight",
...
class NLACriticOutput:
    values: torch.Tensor  # [B, T, d_model] or [N, d_model] when indexed
    backbone_last_hidden: torch.Tensor  # [B, T, d_model] or [N, d_model] when indexed
...
def _truncate_config_layers(config, num_layers: int) -> None:
    ...
    for attr in ("layer_types", "sliding_window_pattern", "no_rope_layers", "hybrid_override_pattern"):
        v = getattr(config, attr, None)
        if isinstance(v, (list, tuple, str)) and len(v) > num_layers:
...
def _inner_transformer(backbone: PreTrainedModel) -> nn.Module:
    ...
    Nemotron-H/Nano: backbone.backbone
    ...
    raise AssertionError(
        f"{type(backbone).__name__} has neither .model, .backbone, nor .transformer — "
        f"add the attribute name here if supporting a new arch"
    )


def _auto_map_module_names(auto_map) -> set[str]:
    names: set[str] = set()
    if isinstance(auto_map, dict):
        values = auto_map.values()
    ...
    for value in values:
        if isinstance(value, str):
            module_path = value.split("--")[-1].split(".")[0]
            if module_path:
                names.add(module_path)
        elif isinstance(value, (list, tuple, dict)):
            names.update(_auto_map_module_names(value))
    return names


def _copy_remote_code_files_from_config(config, save_directory: str | os.PathLike[str]) -> None:
    auto_map = getattr(config, "auto_map", None)
    module_names = _auto_map_module_names(auto_map)
    if not module_names:
        return

    source_name = getattr(config, "_name_or_path", None)
    if not source_name:
        return

    source = Path(source_name)
    if not source.exists():
        source = Path(snapshot_download(str(source_name), allow_patterns=[f"{name}.py" for name in module_names]))

    output = Path(save_directory)
    output.mkdir(parents=True, exist_ok=True)
    for name in module_names:
        src = source / f"{name}.py"
        if src.is_file():
            dst = output / src.name
...
@contextmanager
def _coerce_tied_weight_keys_for_save(module: nn.Module):
    """Transformers 5.x expects `_tied_weights_keys` to be dict-like in save.

    Nano's remote-code model exposes the older list shape. It is not tied
    (`tie_word_embeddings=false`), but save_pretrained still walks the field.
    Temporarily coerce only for save, then restore the exact original objects.
    """
    patched: list[tuple[nn.Module, list[str]]] = []
    for child in module.modules():
        tied = getattr(child, "_tied_weights_keys", None)
        if isinstance(tied, list):
            patched.append((child, tied))
            child._tied_weights_keys = {key: key for key in tied}
    try:
        yield
    finally:
        for child, tied in patched:
            child._tied_weights_keys = tied
...
class NLACriticModel(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *, nla_num_layers: int | None = None, **kwargs):
        ...
        for attr in ("norm", "final_layernorm", "ln_f", "norm_f"):
    ...
    def forward(self, input_ids=None, position_ids=None, attention_mask=None, nla_value_indices=None, **kwargs):
        ...
        if nla_value_indices is not None:
            if h.shape[0] == 1:
                h = h.squeeze(0)
                h_selected = h.index_select(0, nla_value_indices.to(h.device))
            else:
                assert nla_value_indices.ndim == 1 and nla_value_indices.shape[0] == h.shape[0], (
                    f"batched nla_value_indices must be [B], got {tuple(nla_value_indices.shape)} "
                    f"for hidden layout {tuple(h.shape)}"
                )
                batch_rows = torch.arange(h.shape[0], device=h.device)
                h_selected = h[batch_rows, nla_value_indices.to(h.device)]
            return NLACriticOutput(values=self.value_head(h_selected), backbone_last_hidden=h_selected)
    ...
    def save_pretrained(self, save_directory, state_dict=None, **kwargs):
        ...
        with _coerce_tied_weight_keys_for_save(self.backbone):
            self.backbone.save_pretrained(save_directory, state_dict=backbone_sd, **kwargs)
        _copy_remote_code_files_from_config(self.backbone.config, save_directory)
