...
_FAILED_EXTRACTION_REWARD_RAW = os.environ.get("NLA_FAILED_EXTRACTION_REWARD")
...
def failed_extraction_reward() -> float:
    ...
    default = -math.log(2.0) if _USE_LOG_MSE_REWARD else -2.0
    raw = os.environ.get("NLA_FAILED_EXTRACTION_REWARD", _FAILED_EXTRACTION_REWARD_RAW)
    ...
    if not math.isfinite(value):
        raise ValueError(f"NLA_FAILED_EXTRACTION_REWARD must be finite, got {raw!r}")
    if value > default:
        raise ValueError(
            "NLA_FAILED_EXTRACTION_REWARD must not be greater than the default "
            f"{default}; failed parses must not outrank poor valid reconstructions"
        )
    return value


FAILED_EXTRACTION_REWARD = failed_extraction_reward()
...
def _ensure_padding_token(tokenizer) -> None:
    ...
    for token_attr, id_attr in (("eos_token", "eos_token_id"), ("unk_token", "unk_token_id")):
        ...
        if getattr(tokenizer, "pad_token_id", None) is None and token_id is not None:
            tokenizer.pad_token_id = token_id
        return
    raise ValueError("reward tokenizer needs a pad/eos/unk token for batched critic padding")
...
def _lazy_init(args):
    ...
    _ensure_padding_token(_TOKENIZER)
