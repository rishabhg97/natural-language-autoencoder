...
def _as_engine_url(addr: str) -> str:
    addr = addr.strip()
    if not addr:
        return ""
...
def _engine_urls_from_args(args) -> list[str]:
    urls: list[str] = []
    for attr in (
        ...
        if isinstance(raw, str):
            ...
            values = list(raw)
        urls.extend(url for url in (_as_engine_url(str(value)) for value in values) if url)
    return urls
...
async def _resolve_url(args, sample_index: int) -> str:
    ...
    if _ENGINE_URLS is None:
        async with _ENGINE_URLS_LOCK:
            if _ENGINE_URLS is None:
                urls = _engine_urls_from_args(args)
                source = "external engine args"
                if not urls:
                    urls = await get_worker_urls(args)
                    source = "router /workers"
                assert urls, "router /workers returned empty — engines not registered yet"
                _ENGINE_URLS = urls
                _logger.info(f"[NLA] bypassing router via {source}, {len(urls)} engines: {urls}")
...
async def generate(args, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    ...
    # Parsing can relabel a truncated generation as FAILED below. Preserve the
    # engine result so rollout telemetry and guards can distinguish generation
    # truncation from other parse failures.
    sample.nla_generation_status = getattr(sample.status, "name", str(sample.status))
