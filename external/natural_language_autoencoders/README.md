...
> **Vendoring notice:** this directory is the Nano30B production fork, not a
> pristine copy of the reference repository. The imported tree did not retain
> an upstream commit identifier. Nano-specific changes include architecture
> adapters and changes to injection, reward/loss handling, telemetry, rollout,
> and Miles integration. The Nano recipe also uses injection scale `75` rather
> than the reference recipe's `150`, and the current RL design freezes the AR
> critic. See [`NANO_FORK.md`](NANO_FORK.md) before making method-parity claims.
