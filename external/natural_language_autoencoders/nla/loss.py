...
def nla_critic_loss(args, parallel_state, batch, values, sum_of_sample_mean):
    ...
    if values.ndim == 2:
        # Optimized FSDP critic path: train_actor gathered last-token hidden rows
        # before applying the value head, so values is already [B, d].
        assert values.shape[0] == B, f"indexed values has B={values.shape[0]}, expected {B}"
        pred = values
        last_idx = None
    else:
    ...
    gold_norm = gold.norm(dim=-1)
    cosine = F.cosine_similarity(pred.float(), gold.float(), dim=-1)
    log = {
        "loss": loss.detach(),
        "pred_norm_raw": pred_norm.sum().detach(),
        "gold_norm_raw": gold_norm.sum().detach(),
        "cosine_sum": cosine.sum().detach(),
        "pred_norm_ratio_sum": (pred_norm / gold_norm.clamp_min(1e-12)).sum().detach(),
        "mse_scale": torch.tensor(float(B) * (mse_scale if mse_scale is not None else -1.0), device=dev),
    }
    value_head_weight_norm = batch.get("_nla_value_head_weight_norm")
    if value_head_weight_norm is not None:
        log["value_head_weight_norm"] = value_head_weight_norm.detach() * B
    if backbone_h is not None:
        if backbone_h.ndim == 2:
            log["backbone_norm_raw"] = backbone_h.norm(dim=-1).sum().detach()
