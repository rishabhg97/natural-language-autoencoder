...
def _prepare_tokenizer_for_preflight(tokenizer) -> None:
    """Match NLA critic train/eval padding before padded reward-path checks."""
    tokenizer.padding_side = "right"
    if getattr(tokenizer, "pad_token_id", None) is None:
        eos_token = getattr(tokenizer, "eos_token", None)
        assert eos_token is not None, (
            "tokenizer has no pad_token and no eos_token; cannot build padded "
            "critic preflight batches"
        )
        tokenizer.pad_token = eos_token


def _last_real_token_indices(attention_mask: torch.Tensor) -> torch.Tensor:
    """Return the rightmost real-token index for each padded row."""
    return attention_mask.cumsum(dim=1).argmax(dim=1)


def _predict_padded_reward_path(
    model: torch.nn.Module, input_ids: torch.Tensor, attention_mask: torch.Tensor
) -> torch.Tensor:
    """Reward path: run padded batch, then index the last real-token values."""
    last_idx = _last_real_token_indices(attention_mask)
    with torch.no_grad():
        values = model(input_ids=input_ids, attention_mask=attention_mask, use_cache=False).values
        rows = torch.arange(input_ids.shape[0], device=input_ids.device)
        return values[rows, last_idx].float().cpu()


def _predict_padded_train_path(
    model: torch.nn.Module, input_ids: torch.Tensor, attention_mask: torch.Tensor
) -> torch.Tensor:
    """Current Nano critic-train path: padded batch with selected value rows."""
    last_idx = _last_real_token_indices(attention_mask)
    with torch.no_grad():
        values = model(
            input_ids=input_ids,
...
def check_critic_paths_and_scale(critic_hf_dir: str, tol: float) -> None:
    ...
    _prepare_tokenizer_for_preflight(tok)
    ...
    pred_rwd = _predict_padded_reward_path(m, ids, mask)

    # ─── current training path: padded+masked batch with selected value rows ───
    # Nano/Nemotron-H packed THD critic training is intentionally not the
    # default gate: post-audit AR training uses padded critic microbatches
    # because packed THD was not numerically equivalent for this architecture.
    pred_train = _predict_padded_train_path(m, ids, mask)
    ...
    if max_dev > tol:
        ...
        _die(
            ...
            f"Check last_idx vs padding_side, use_cache, nla_value_indices, "
            f"or normalize_activation/mse_scale drift."
        )
