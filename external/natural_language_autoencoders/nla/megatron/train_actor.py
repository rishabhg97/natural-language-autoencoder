...
def _force_constant_lr_for_stale_resume(args) -> bool:
    """Return whether to preserve the old stale-scheduler constant-LR override."""

    if _env_flag_enabled("NLA_FORCE_CONSTANT_LR"):
        return True
    if _env_flag_enabled("NLA_PRESERVE_LR_SCHEDULE"):
        return False
    schedule = str(getattr(args, "lr_decay_style", "constant") or "constant").lower()
    if schedule in {"", "constant", "none"}:
        return True
    if getattr(args, "finetune", False) or getattr(args, "no_load_optim", False):
        return False
    return bool(getattr(args, "load", None))


def _apply_live_lr_policy(opt_param_scheduler, optimizer, args) -> None:
    """Clear checkpoint-stale LR bounds while preserving requested decay when safe."""

    lr = float(args.lr)
    requested_min_lr = float(getattr(args, "min_lr", 0.0) or 0.0)
    if _force_constant_lr_for_stale_resume(args):
        min_lr = lr
        style = "constant"
    else:
        min_lr = requested_min_lr
        style = str(getattr(args, "lr_decay_style", "constant") or "constant")

    opt_param_scheduler.max_lr = lr
    opt_param_scheduler.min_lr = min_lr
    opt_param_scheduler.lr_decay_style = style
    inner = optimizer.optimizer if hasattr(optimizer, "optimizer") else optimizer
    for group in inner.param_groups:
        # get_lr() reads param_group.get('max_lr', self.max_lr); checkpointed
        # param groups can carry stale bounds, so refresh the live groups too.
        group["lr"] = lr
        group["max_lr"] = lr
        group["min_lr"] = min_lr
...
class NLAMegatronActor(MegatronTrainRayActor):
    ...
    def init(self, args, role, with_ref=False):
        ...
        # Checkpoint-loaded scheduler state can carry stale decay bounds that
        # collapse lr to 0 on resume. Refresh those bounds, but keep requested
        # cosine/linear decay for fresh SFT runs; force constant only for stale
        # optimizer resumes or explicit NLA_FORCE_CONSTANT_LR=1.
        _apply_live_lr_policy(self.opt_param_scheduler, self.optimizer, args)
    ...
    def _train_nla_critic(self, rollout_id, rollout_data):
        ...
        if self.role == "critic":
            ...
            assert repartition_report is not None, (
                "online critic rollout is missing its balanced repartition report"
            )
            minimum = float(
            ...
            assert post_filter_retained == int(repartition_report["retained_samples"]), (
                "critic safety truncation dropped rows after balanced repartition: "
                f"expected={repartition_report['retained_samples']} "
